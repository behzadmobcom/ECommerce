...
namespace API.Utilities
{
    public static class FileUtility
    {
        ...
        private static string GetUploadDestination(string fileName, double timeDifference)
        {
            ...
            var retValue = Path.Combine(dayPath, $"{GetFileName(fileName)}_{DateTime.Now.AddMinutes(timeDifference).Ticks.ToString().Substring(0, 5)}.{GetFileExtension(fileName)}");
            return retValue;
        }
