...
namespace API.Repository
{
    public class ProductAttributeRepository : AsyncRepository<ProductAttribute>, IProductAttributeRepository
    {
        ...
        public async Task<PagedList<ProductAttribute>> Search(PaginationParameters paginationParameters, CancellationToken cancellationToken)
        {
            return PagedList<ProductAttribute>.ToPagedList(await _context.ProductAttributes.Where(x => x.Title.Contains(paginationParameters.Search)).AsNoTracking().OrderBy(on => on.Id).ToListAsync(cancellationToken),
                paginationParameters.PageNumber,
                paginationParameters.PageSize);
        }
        public async Task<ProductAttribute> GetByTitle(string title, CancellationToken cancellationToken) => await _context.ProductAttributes.Where(x => x.Title == title).FirstOrDefaultAsync(cancellationToken);
        ...
        public async Task<IEnumerable<ProductAttribute>> GetAllAttributeWithGroupId(int groupId,int productId, CancellationToken cancellationToken)
        {
            var productAttributes =await  _context.ProductAttributes.Where(x=>x.AttributeGroupId== groupId)
                .Include(i=>i.AttributeValue)
                .Select(s=>new ProductAttribute{Id = s.Id,AttributeValue = s.AttributeValue.Where(v=>v.ProductId==productId).ToList(),AttributeGroup = s.AttributeGroup,AttributeGroupId = s.AttributeGroupId,AttributeType = s.AttributeType,Title = s.Title})
