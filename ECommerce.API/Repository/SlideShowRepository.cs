...
namespace API.Repository
{
    public class SlideShowRepository : AsyncRepository<SlideShow>, ISlideShowRepository
    {
        ...
        public async Task<SlideShow> GetByTitle(string title, CancellationToken cancellationToken) => await _context.SlideShows.Where(x => x.Title == title).FirstOrDefaultAsync(cancellationToken);
