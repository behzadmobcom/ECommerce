...
namespace API.Repository
{
    public class BlogRepository : AsyncRepository<Blog>, IBlogRepository
    {
        ...
        public async Task<PagedList<Blog>> Search(PaginationParameters paginationParameters, CancellationToken cancellationToken)
        {
            return PagedList<Blog>.ToPagedList(await _context.Blogs.Where(x => x.Title.Contains(paginationParameters.Search)).AsNoTracking().OrderBy(on => on.Id).ToListAsync(cancellationToken),
                paginationParameters.PageNumber,
                paginationParameters.PageSize);
        }

        public async Task<Blog> GetByTitle(string title, CancellationToken cancellationToken) => await _context.Blogs.Where(x => x.Title == title).FirstOrDefaultAsync(cancellationToken);
        ...
        public async Task<Blog> AddWithRelations(BlogViewModel blogViewModel, CancellationToken cancellationToken)
        {
            ...
            foreach (var id in blogViewModel.KeywordsId)
            {
                blog.Keywords.Add(await _context.Keywords.FindAsync(id));
            }
            blog.Tags = new List<Tag>();
            foreach (var id in blogViewModel.TagsId)
            {
                blog.Tags.Add(await _context.Tags.FindAsync(id));
            }
        ...
        {
            var blog = await _context.Blogs.Where(x => x.Id == blogViewModel.Id).
            ...
            foreach (var blogKeyword in blog.Keywords)
            {
                blog.Keywords.Remove(blogKeyword);
            }
            foreach (var id in blogViewModel.KeywordsId)
            {
                blog.Keywords.Add(await _context.Keywords.FindAsync(id));
        ...
        public async Task<Blog> GetByUrl(string url, CancellationToken cancellationToken) => await _context.Blogs.Where(x => x.Url == url).FirstOrDefaultAsync(cancellationToken);
    }
}
