...
namespace API.Repository
{
    public class UserRepository : AsyncRepository<User>, IUserRepository
    {
        ...
        { }
        public async Task<PagedList<User>> Search(PaginationParameters paginationParameters, CancellationToken cancellationToken)
        {
            return PagedList<User>.ToPagedList(await TableNoTracking.Where(x => x.UserName.Contains(paginationParameters.Search)).AsNoTracking().OrderBy(on => on.Id).ToListAsync(cancellationToken),
                paginationParameters.PageNumber,
                paginationParameters.PageSize);
        }
        public async Task<User> GetByEmailOrUserName(string input, CancellationToken cancellationToken)
        {
            return await TableNoTracking.Where(p => p.Email == input || p.PhoneNumber == input || p.UserName == input).Include(x=>x.UserRole).SingleOrDefaultAsync(cancellationToken);
        }
        ...
        {
            return await TableNoTracking.AnyAsync(p => p.Id != id && (p.Email == email || p.PhoneNumber == phoneNumber), cancellationToken);
        }
