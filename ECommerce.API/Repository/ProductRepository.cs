...
namespace API.Repository
{
    public class ProductRepository : AsyncRepository<Product>, IProductRepository
    {
        ...
        public async Task<Product> GetByName(string name, CancellationToken cancellationToken) => await _context.Products.Where(x => x.Name == name).FirstOrDefaultAsync(cancellationToken);

        public async Task<Product> GetByUrl(string url, CancellationToken cancellationToken) => await _context.Products.Where(x => x.Url == url).Include(x => x.Brand).Include(x => x.Keywords).Include(t => t.Tags).Include(x => x.Images).Include(x => x.Prices).FirstOrDefaultAsync(cancellationToken);
        ...
        public async Task<Product> AddWithRelations(ProductViewModel productViewModel, CancellationToken cancellationToken)
        {
            ...
            {
                product.Keywords.Add(_context.Keywords.First(x => x.Id == id));
            }
            product.Tags = new List<Tag>();
            foreach (var id in productViewModel.TagsId)
            {
                product.Tags.Add(_context.Tags.First(x => x.Id == id));
            }
        ...
        public async Task<Product> EditWithRelations(ProductViewModel productViewModel, CancellationToken cancellationToken)
        {
            var product = await _context.Products.Where(x => x.Id == productViewModel.Id).
                Include(nameof(Product.ProductCategories)).
                Include(nameof(Product.Tags)).
                Include(nameof(Product.Keywords)).
                Include(nameof(Product.Images)).
                Include(nameof(Product.AttributeValues)).
                Include(nameof(Product.AttributeGroupProducts)).
            ...
            foreach (var productKeyword in product.Keywords)
            {
                product.Keywords.Remove(productKeyword);
            }
            foreach (var id in productViewModel.KeywordsId)
            {
            ...
            foreach (var id in productViewModel.TagsId)
            {
                product.Tags.Add(await _context.Tags.FirstAsync(x => x.Id == id));
            }
        ...
        public async Task<Product> GetProductByIdWithInclude(int productId, CancellationToken cancellationToken)
        {
            return await _context.Products.AsNoTracking().
                ...
                .Include(st => st.Store)
                .Include(i => i.Images).FirstOrDefaultAsync(cancellationToken);
        }
        ...
        public async Task<List<ProductIndexPageViewModel>> GetProductList(List<int> productIdList, CancellationToken cancellationToken)
        {
        ...
        public async Task<List<ProductCompareViewModel>> GetProductListWithAttribute(List<int> productIdList, CancellationToken cancellationToken)
        {
            ...
            var products = await _context.Products.Where(x => productIdList.Contains(x.Id)).Include(x => x.Prices)
                  ...
                  .ToListAsync(cancellationToken);
            productCompareViewModel.AddRange(products.Select(product => (ProductCompareViewModel)product));
        ...
        {
            var products = await _context.Products.Where(x => x.ProductCategories.Any(y => y.Id == categoryId)).OrderByDescending(x => x.Id)
               .Include(x => x.Brand)
               .Include(x => x.Images)
               .Include(x => x.Prices)
               .Include(x => x.ProductUserRanks)
        ...
        public IQueryable<Product> GetProducts(List<int>? brandsId, List<int>? starsCount, List<int>? tagsId)
        {
            ...
            if (brandsId is {Count: > 0})
            {
                products = products.Where(x => brandsId.Contains((int)x.BrandId));
            }

            if (starsCount is { Count: > 0 })
            {
                products = products.Where(x => starsCount.Contains(x.ProductUserRanks.Sum(s => s.Stars) / x.ProductUserRanks.Count));
            }
            if (tagsId is { Count: > 0 })
            {
                products = tagsId.Aggregate(products, (current, tagId) => current.Where(x => x.Tags.Any(t => t.Id == tagId)));
            }
        ...
        public async Task<List<Product>> TopNew(int count, CancellationToken cancellationToken)
        {
        ...
        public async Task<List<Product>> TopStars(int count, CancellationToken cancellationToken)
        {
            ...
            if (products.Count < 5)
            {
                products = await TopChip(count - products.Count, cancellationToken);
            }
        ...
        {
            var products = await _context.ProductSellCounts.
                OrderByDescending(x => x.Count).
                Take(count).Select(x => x.Product)
                .ToListAsync(cancellationToken);

            if (products == null)
            {
                products = await _context.Products.OrderByDescending(x => x.Prices.Any(p => p.IsDefault && p.Exist > 0)).Take(count)
                    .ToListAsync(cancellationToken);
            }
