...
namespace API.Controllers
{
    ...
    public class UnitsController : ControllerBase
    {
        ...
        public UnitsController(IUnitRepository unitRepository, IHolooUnitRepository holooUnitRepository, ILogger<UnitsController> logger)
        {
        ...
        public async Task<ActionResult<bool>> Put(Unit unit, CancellationToken cancellationToken)
        {
            try
            {
                if (unit.Id == 1) return Ok(new ApiResult
                {
                    Code = ResultCode.BadRequest,
        ...
        public async Task<IActionResult> Delete(int id, CancellationToken cancellationToken)
        {
            try
            {
                if (id == 1) return Ok(new ApiResult
                {
                    Code = ResultCode.BadRequest,
        ...
        public async Task<IActionResult> ConvertHolooToSunflower(CancellationToken cancellationToken)
        {
            try
            {
                IEnumerable<Unit> units = (await _holooUnitRepository.GetAll(cancellationToken))!.
                    Select(x => new Unit()
                    {
                        Name = x.Unit_Name,
                        Few = x.Unit_Few,
                ...
                catch (Exception e)
                {
                    ...
                    return Ok(new ApiResult { Code = ResultCode.DatabaseError,  Messages = new List<string> { "افزودن اتوماتیک به مشکل برخورد کرد" } });
                }
                return Ok(new ApiResult
                {
                    Code = ResultCode.Success
