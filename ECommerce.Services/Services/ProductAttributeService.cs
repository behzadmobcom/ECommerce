...
namespace Services.Services
{
    public class ProductAttributeService : EntityService<ProductAttribute>, IProductAttributeService
    {
        ...
        public async Task<ServiceResult<List<ProductAttribute>>> Load(int productId=0, int pageNumber = 0, int pageSize = 10)
        {
            var result = productId == 0 ? await ReadList(Url) : await ReadList(Url,$"GetAll/{productId}");
            return Return<List<ProductAttribute>>(result);
        }
        ...
        public async Task<ServiceResult<List<ProductAttribute>>> GetAllAttributeWithGroupId(int attributeGroupsId,int productId)
        {
        ...
        public async Task<ServiceResult<ProductAttribute>> GetById(int id)
        {
            ...
            return Return<ProductAttribute>(result);
        }
