...
namespace Services.Services
{
    public class EntityService<T> : IEntityService<T>
    {
        ...
        public async Task<ApiResult<T>> Read(string url, string api)
      {
        ...
        {
            var response = await _http.PostAsync<T>(url, entity);
            if (response == null)
                return new ApiResult<object>{Code = ResultCode.ServerDontResponse,Messages = new List<string>{ "سرور سایت در دسترس نیست. لطفا با پشتیبان سایت تماس بگیرید" }};
            response.Messages = response.Code > 0 ? new List<string>{response.GetBody()} : new List<string> { "با موفقیت ذخیره شد" };
            return response;
        }
        ...
        public async Task<ApiResult> UpdateWithReturnId(string url, T entity)
        {
            ...
            var messages = response.Code > 0 ? new List<string> { response.GetBody() } : new List<string> { "با موفقیت ویرایش شد" };
        ...
        {
            var response = await _http.PutAsync(url, entity);
            response.Messages = response.Code > 0 ? new List<string> { response.GetBody() } : new List<string> { "با موفقیت ویرایش شد" };
            return response;
        }
        ...
        public async Task<ApiResult> Delete(string url, int entityId)
        {
            ...
            response.Messages = response.Code > 0 ? new List<string> { response.GetBody() } : new List<string> { "با موفقیت حذف شد" };
            return response;
        }
