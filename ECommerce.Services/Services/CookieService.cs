    ...
    public class CookieService : ICookieService
    {
            ...
            if (startWith)
            {
                ...
                foreach (var cookie in cookies)
                {
                    if (!cookie.Key.StartsWith(key))
                        continue;
                    productIds.Add(new CookieData(cookie.Key,value: Convert.ToInt32(cookie.Value)));
                }
            }
            else
            {
        ...
        public LoginViewModel GetCurrentUser()
        {
            try
            {
                ...
                {
                    Id = Convert.ToInt32(jwtSecurityToken.Claims.First(claim => claim.Type == ClaimTypes.NameIdentifier).Value),
                    ...
                    IsColleague = Convert.ToBoolean(jwtSecurityToken.Claims.First(claim => claim.Type == "IsColleague").Value)
                };
